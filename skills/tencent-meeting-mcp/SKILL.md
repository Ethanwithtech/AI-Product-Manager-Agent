    ...
    "openclaw":
      {
        ...
        "tokenUrl": "https://work-test.medialab.qq.com/test/stke18075env/mcp/wemeet-open/v1",
        "emoji": "📅"
      }
  }
---
...
- **完整时间处理流程**：
  1. 调用 `convert_timestamp`（不传参数）→ 获取当前时间/昨天/一周前等基准时间
  2. 基于基准时间推算目标时间字符串
  3. 调用 `convert_timestamp`（传入 `parse_time_str`）→ 获取目标时间戳
  4. 将时间戳传给会议工具
...
- 在用户要求获取**智能纪要访问地址**时，使用 `get_smart_minutes_address`
...
3. **时间戳 → 时间字符串**：传入 `parse_time_unix`（如 `1774692000`），获取对应的可读时间字符串

**参数**（二选一，也可都不传）:
- `parse_time_str`(可选) - 待解析的时间字符串，格式：`YYYY-MM-DD HH:MM:SS`（如 `2026-03-25 14:00:00`）。传入时返回对应的秒级 Unix 时间戳
- `parse_time_unix`(可选) - 待解析的秒级 Unix 时间戳（如 `1774594800`）。传入时返回对应的可读时间字符串
- 两个参数**不能同时填写**；**都不填时**，工具自动以当前时间作为解析对象

**返回值**: JSON格式，包含以下字段：
- `parsed_time_str` - 解析后的时间字符串（格式：`YYYY-MM-DD HH:MM:SS`）
- `parsed_time_unix` - 解析后的秒级时间戳
- `time_now_str` - 当前北京时间字符串
- `time_now_unix` - 当前北京时间的秒级时间戳
...
#### `get_smart_minutes_address` — 获取智能纪要访问地址

**参数**: 
- `meeting_record_id`(必填) - 会议录制ID
- `lang`(可选) - 翻译选择：default(原文，不翻译) / zh(简体中文) / en(英文) / ja(日语)

> 调用示例请参考：`references/api_references.md` - get_smart_minutes_address
