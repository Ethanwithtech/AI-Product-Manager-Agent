...
3. **本地标准 WAV 的默认大文件策略是切片后逐片走 Flash**
   - 只有明确命中异步 / URL / 高级结果格式需求时，才走 `file_recognize.py rec`
...
1. 如果同时满足：
   ...
   那么使用：
   ```bash
   python3 <SKILL_DIR>/scripts/sentence_recognize.py "<PUBLIC_URL>"
   ```
...
3. 如果：
   ...
   那么 `file_recognize.py rec` 也不适用，转到“本地处理链”
...
3. 如果同时满足：
   ...
   ```
   这是“本地大文件的优先优化分支”：它可以避免切片和逐片拼接，但不能突破 `CreateRecTask` 的 `5h` 限制。
...
除“公网 URL 且已合规”的优先路径外，只有命中以下任一条件，才允许主动选择 `file_recognize.py rec`：
